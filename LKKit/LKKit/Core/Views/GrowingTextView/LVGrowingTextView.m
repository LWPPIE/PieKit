...
@implementation LVGrowingTextView
...
- (void)setPlaceholder:(NSString *)placeholder {
    ...
    [_placeholderLabel setLeft:_contentInset.left + 6.0];
    [self showOrHidePlaceholder];
}

- (UILabel *)placeholderLabel {
    if (!_placeholderLabel) {
        _placeholderLabel = [[UILabel alloc] initWithFrame:CGRectInset(self.bounds, _contentInset.left + 6.0, 0.0)];
        _placeholderLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
        _placeholderLabel.font = _internalTextView.font;
        _placeholderLabel.textColor = [UIColor lightGrayColor];
        _placeholderLabel.backgroundColor = [UIColor clearColor];
        [self insertSubview:_placeholderLabel aboveSubview:_internalTextView];
    }
...
- (void)insertAttributedString:(NSAttributedString *)attrString atIndex:(NSUInteger)loc
{
    if (_internalTextView) {
        [_internalTextView.textStorage insertAttributedString:attrString atIndex:loc];
    }
...
{
    if (_internalTextView) {
        [_internalTextView deleteBackward];
    }
...
- (void)showOrHidePlaceholder {
    if (_internalTextView.text && _internalTextView.text.length > 0) {
        self.placeholderLabel.hidden = YES;
    }else if(_internalTextView.textStorage && _internalTextView.textStorage.length > 0) {
        self.placeholderLabel.hidden = YES;
    }else {
