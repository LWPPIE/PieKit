...
@property (nonatomic, weak) FTCoreTextNode *supernode;
...
- (FTCoreTextNode*)getNodeThatContainLocationFormNSRange:(NSRange)range
{
    ...
    NSUInteger i = 0;
    NSUInteger count = [currentNode.subnodes count];
...
- (NSString *)getIndexingForNode:(FTCoreTextNode*)node
{
    ...
    do {
        ...
        {
            string = [NSString stringWithFormat:@"%@%@",@(currentNode.nodeIndex),string];
        }
        else
        {
            string = [NSString stringWithFormat:@"/%@%@",@(currentNode.nodeIndex),string];
				...
				for (NSString *key in urlsKeys) {
					...
					if (index >= range.location && index < range.location + range.length) {
						...
						if (activeRects && _highlightTouch) {
							...
							for (NSUInteger iLine = startLineIndex; iLine <= endLineIndex; iLine++) {
								CTLineRef line = (__bridge CTLineRef)[lines objectAtIndex:iLine];
								CGFloat ascent, descent;
								CGFloat lineWidth = CTLineGetTypographicBounds(line, &ascent, &descent, NULL);
	...
	while (!finished) {
		...
		if (tagRange.location == NSNotFound) {
			if (currentSupernode != rootNode && !currentSupernode.isClosed) {
                if (_verbose) NSLog(@"FTCoreTextView :%@ - Couldn't parse text because tag '%@' at position %@ is not closed - aborting rendering", self, currentSupernode.style.name, @(currentSupernode.startLocation));
				return;
			}
			finished = YES;
            continue;
